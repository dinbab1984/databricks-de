...
def write_twice(microBatchDF, batchId):
    ...
    microBatchDF.select("id", "name", F.current_timestamp().alias("processed_time")).write.mode("append").saveAsTable("silver_name")
    
    microBatchDF.select("id", "value", F.current_timestamp().alias("processed_time")).write.mode("append").saveAsTable("silver_value")
